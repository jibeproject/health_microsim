...
healthy_agegroup_cycle  <- all_data |> 
  filter(value == "healthy") |> 
...
healthy_age_counts_agegroup_cycle  <- all_data |> 
  filter(value == "healthy", cycle >= MIN_CYCLE) |>
  group_by(scen, cycle, agegroup_cycle) |> 
  summarise(num = n_distinct(id), .groups = "drop") |> 
  collect()

healthy_age_counts_imd  <- all_data |> 
  filter(value == "healthy", cycle >= MIN_CYCLE) |>
  left_join(zones |> 
              dplyr::select(lsoa21cd, imd10) |> 
              distinct(), copy = T) |> 
...
asr_agegroup_cycle_all <- calc_asr(incidence, all_causes, people_raw, W_OVERALL,
                        group_vars = "agegroup_cycle", avg_cycles = NULL, min_cycle = MIN_CYCLE)
asr_agegroup_cycle_all_avg_1_30 <- calc_asr(incidence, all_causes, people_raw, W_OVERALL,
                                 group_vars = "agegroup_cycle", avg_cycles = MIN_CYCLE:MAX_CYCLE, min_cycle = MIN_CYCLE)
...
pc <- mget(c(
  ...
  "asr_agegroup_cycle_all","asr_agegroup_cycle_all_avg_1_30",
