...
get_summary <- function(SCEN_NAME, group_vars = NULL){
  ...
  if (!FILE_PATH_BELEN){
    ...
    m <- arrow::open_dataset(paste0("manchester/health/processed/", SCEN_NAME, "_dis_inter_state_trans-n.c-", n.c, "-n.i-", n.i, "-n.d-19.parquet"))
  }
...
reference_df_healthy <- dc_lad_overtime %>% filter(value == "healthy", scen == "reference")

# Join the reference data with the original data to calculate the change in count
df_change_lad_overtime_healthy <- dc_lad_overtime |> 
  filter(value == "healthy") |>
  left_join(reference_df_healthy, by = c("ladcd", "cycle"), suffix = c("", "_ref")) |>
...
g <- ggplot(df_change_lad_overtime_healthy, aes(x = factor(cycle, levels = paste0("c", 0:n.c)), y = count_change, color = scen, group = scen)) +
  geom_line() +
  geom_point() +
  facet_wrap(~ ladnm) +
...
reference_df_lad_overtime_alive <- dc_lad_overtime %>% filter(value != "dead", scen == "reference") |> group_by(cycle, ladcd, scen) |> summarise(count = sum(count)) # futher grouping here as there are people in diff health states
# Join the reference data with the original data to calculate the change in count
df_change_lad_overtime_alive <- dc_lad_overtime |>
  filter(value != "dead") |>
  group_by(cycle, ladcd, scen) |> summarise(count = sum(count)) |>
  ungroup() |>
  left_join(reference_df_lad_overtime_alive, by = c("ladcd", "cycle"), suffix = c("", "_ref")) |>
  mutate(count_change = count - count_ref) |>
  dplyr::select(ladcd, cycle, scen, count_change) |>
  left_join(zones)
# Create a line plot showing the difference for the "alive" value, faceted by ladcd
g <- ggplot(df_change_lad_overtime_alive, aes(x = factor(cycle, levels = paste0("c", 0:n.c)), y = count_change, color = scen, group = scen)) +
  geom_line() +
  geom_point() +
  facet_wrap(~ ladnm) +
...
reference_df_cumulative_lad_healthy <- dc_cumulative_lad %>% filter(value == "healthy", scen == "reference")

df_change_cumulative_lad_healthy <- dc_cumulative_lad |> 
  filter(value == "healthy") |>
  left_join(reference_df_cumulative_lad_healthy, by = "ladcd", suffix = c("", "_ref")) |>
  mutate(count_change = count - count_ref) |>
...
df_change_cumulative_lad_healthy <- df_change_cumulative_lad_healthy %>% filter(scen != "reference")

# Create the bar chart
g <- ggplot(df_change_cumulative_lad_healthy, aes(x = scen, y = count_change, fill = scen)) +
...
reference_df_cumulative_lad_alive <- dc_cumulative_lad %>% filter(value != "dead", scen == "reference") |> group_by(ladcd, scen) |> summarise(count = sum(count)) # futher grouping here as there are people in diff health states
# Join the reference data with the original data to calculate the change in count
df_change_df_cumulative_lad_alive <- dc_cumulative_lad |>
  filter(value != "dead") |>
  group_by(ladcd, scen) |> summarise(count = sum(count)) |>
  ungroup() |>
  left_join(reference_df_cumulative_lad_alive, by = c("ladcd"), suffix = c("", "_ref")) |>
  mutate(count_change = count - count_ref) |>
...
df_change_df_cumulative_lad_alive <- df_change_df_cumulative_lad_alive %>% filter(scen != "reference")


g <- ggplot(df_change_df_cumulative_lad_alive, aes(x = scen, y = count_change, fill = scen)) +
  geom_bar(stat = "identity") +
  facet_wrap(~ ladnm, scales = "free") +
...
reference_df_cumulative_incidence <- dc_cumulative_lad %>% filter(!value %in% c("dead", "healthy"), scen == "reference")

# Join the reference data with the original data to calculate the change in count
df_change_cumulative_incidence <- dc_cumulative_lad |>
  filter(!value %in% c("dead", "healthy")) |>
  left_join(reference_df_cumulative_incidence, by = c("ladcd", "value"), suffix = c("", "_ref")) |>
  mutate(count_change = count - count_ref) |>
...
df_change_cumulative_incidence <- df_change_cumulative_incidence %>% filter(scen != "reference") %>% 
                filter(value %in% c("all_cause_dementia" , "coronary_heart_disease",
                                    "stroke", "diabetes", "depression"))


g <- ggplot(df_change_cumulative_incidence, aes(x = scen, y = count_change, fill = value)) +
