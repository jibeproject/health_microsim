#' Calculate relative risk given NDVI exposure level
#'
#' Calculate the relative risk (RR) for each person in the baseline population for each
#' disease related to air pollution and each scenario based on the individual NDVI exposure levels
#'
#' This function performs the following steps:
#'
#' \itemize{
#' \item various checks to ensure the correct NDVI exposure levels (doses) and disease are read in
#'
#' \item get the lookup table for the required dose response functions which contains for each dose
#'   the median RR values
#'
#' \item for the NDVI exposure doses in the baseline population find the needed RR by extrapolating the
#'   dose responses given in the lookup table
#'
#' }
#'
#' @param cause name of disease
#' @param dose vector of NDVI exposure levels from individuals for a given age range and scenario
#'
#' @return data frame of relative risks for each person in the baseline population and each NDVI related disease in each scenario
#'
#' @export


NDVI_dose_response <- function(cause, dose) {
  ...
  # interpolate the values in the lookup table to get RR values for all NDVI exposure doses
