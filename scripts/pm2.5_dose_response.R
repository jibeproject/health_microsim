#' Calculate relative risk given NO2 exposure level
#'
#' Calculate the relative risk (RR) for each person in the baseline population for each
#' disease related to air pollution and each scenario based on the individual NO2 exposure levels
#'
#' This function performs the following steps:
#'
#' \itemize{
#' \item various checks to ensure the correct NO2 exposure levels (doses) and disease are read in
#'
#' \item get the lookup table for the required dose response functions which contains for each dose
#'   the median RR values
#'
#' \item for the NO2 exposure doses in the baseline population find the needed RR by extrapolating the
#'   dose responses given in the lookup table
#'
#' }
#'
#' @param cause name of disease
#' @param dose vector of NO2 exposure levels from individuals for a given age range and scenario
#'
#' @return data frame of relative risks for each person in the baseline population and each NO2 related disease in each scenario
#'
#' @export


pm2.5_dose_response <- function(cause, dose) {
  ...
  if (!cause %in% c(
    ...
    print("Unsupported cause/disease: ", cause)
    stop("Please select from \n
         all_cause_pm, copd_pm, ihd_pm , lc_pm, stroke_pm")
