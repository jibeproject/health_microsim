...
cvd_dia_copd_socio <- bind_rows(cvd_inc, diabetes_inc, stroke_inc, copd_inc) %>%
...
cvd_dia_copd_inc <- inc_cvd_dia_copd_agesex %>% 
    ...
    left_join(cvd_dia_copd_socio %>% select(cause, sex, socio, rr_socio = rr), 
...
We use GBD data as the age and sex data for prevalence estimates. **Any
exceptions?**
...
-   Stroke and Coronary heart disease:
    `AIHW/cvd-diabetes-comorbidities.xlsx`, Tables 'Stroke' and 'CHD'.
...
-   All cause dementia: `AIHW/dementia-prevalence.xlsx`, Table S2.6.
    ...
    AIHW, by socioeconomic quintile and sex, for 2021.
...
REVISE THIS - COPIED AND PASTED FROM INCIDENCE; TO UPDATE \>\>\>\>\>The
published incidence by age and sex (averaged over deprivation levels) is
shown by the dotted black lines (original gbd data interpolated from 5
year age groups to one year). The estimated incidence disaggregated by
socioeconomic quintile is shown by the coloured lines, with
socioeconomic status indicated by the lightness of the lines (calculated
from gbd data and aihw data).
...
```{r}
# plot of data
p <- ggplot(cancer_prevalence_area, ###upo to here - neeed to adapt
#        aes(x=age, y=rate_soc, col=sex, alpha=socio)) + 
#     geom_line(lwd=1.5) + 
#     geom_line(data=inc_cvd_dia_copd_agesex,
#               aes(x=age, y=rate, group=sex), 
...
dementia, diabetes and COPD from AIHW (see sources above), processed in
...
`health/processed/cvd_dem_dia_copd_prevalence_area.rds`, and output
plots are saved in the \`docs/images/prev_cvd_dem_dia_copd' folder.

```{r}
# Stroke

## to do OTHER DISEASES
