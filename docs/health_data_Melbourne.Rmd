...
gbd <- read.csv("docs/health/original/GBD/gbd_incidence.csv")
...
names for the ATLAS data (for now). The sites that we intend to use are
defined by stdname and ICD-10 and these names correspond to the names
use in the physical activity SRMA by Garcia et al, and associated
package. Each row of the table below details the site from the AIHW
tables that these are assumed to be matched to.
...
inc <- read_xlsx("docs/health/original/AIHW/cancer-incidence.xlsx", sheet="Table S1a.1", skip=5, col_names=TRUE) %>% 
...
excel_file <- "docs/health/original/ATLAS/Atlas_estimates_95CIs.xlsx"
...
#ADD population nubmer here

pop_SA2 <- read_csv("docs/health/original/ABS/Population by SA2 area.csv", skip = 9, col_names = TRUE) %>%
...
saveRDS(cancer_incidence_area, file="docs/health/processed/cancer_incidence_area.rds")
...
cancerinc <- readRDS("docs/health/processed/cancer_incidence.rds")
