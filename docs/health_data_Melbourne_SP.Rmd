...
possible using publicly available data.
...
we can compute $r_1$ given $r_{ave}$, the relative rates for the risk factor
$RR_i$ and  opulation proportions for the risk factor $p_i$. Hence we can
compute the remaining $r_i$ by multiplying $r_1$ by $RR_i$.
...
Welfare (AIHW) data and for mortality Australian Bureau of statistic
(ABS) data.
...
created and saved in docs/images/data_prep to visually check the fit of
the interpolation to the original data.
...
# Apply function (select one of the above). Smooth_spline does a good job.
...
output_dir <- "docs/images/cancer_gbd_age_sex/"
...
For cancers, we use data from AIHW, which similar to GBD, will be
interpolated.

#### Harmonise definitions of cancer sites

The cancer sites are reported differently between the AIHW tables on
incidence by age/sex and incidence by socioeconomic quintiles.
...
# Generate and save all pages
output_dir <- "docs/images/cancer_aihw_age_sex/"
if (!dir.exists(output_dir)) {
  dir.create(output_dir, recursive = TRUE)
}
...
https://rmiteduau.sharepoint.com/:x:/r/sites/JIBEworkinggroup/Shared%20Documents/General/health/diseases%20data.xlsx?d=w39d6dfa0db7c4a3294a89cf8ad3f71ee&csf=1&web=1&e=do8erQ
...
folder_path <- file.path("docs", "images", "cancer_inc_compare")
...
for this, so I've used the interpolated file 'gbdpyr'.  And please also note that I've included COPD here along with the other non-cancer diseases, as the structure of the two original .rmd files was so similar.\]**
...
We use the method outlined in `Inequalities in health` above to disaggregate the age and sex data by socioeconomic status.
...
# save plot
# Generate and save all pages
output_dir <- "docs/images/cvd_dia_copd_inc/"
if (!dir.exists(output_dir)) {
  dir.create(output_dir, recursive = TRUE)
}
