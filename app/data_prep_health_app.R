...
all_data <- open_dataset("/run/user/1001/gvfs/smb-share:server=ifs-prod-1152-cifs.ifs.uis.private.cam.ac.uk,share=cedar-grp-drive/Ali/all_data.parquet/") %>% 
  filter(cycle < max_year) %>% 
  collect()
...
zones    <- readr::read_csv("/run/user/1001/gvfs/smb-share:server=ifs-prod-1152-cifs.ifs.uis.private.cam.ac.uk,share=cedar-grp-drive/HealthImpact/Data/Country/UK/JIBE/manchester/input/zoneSystem.csv")
...
qs::qsave(pc, paste0("prec_", max_year, "yrs.qs"))
