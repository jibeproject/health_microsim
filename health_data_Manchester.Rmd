...
compute_asdr <- function(df_deaths, df_pop, by_keys,
                         weights = population_weights_England) {
...
england_lsoa_deaths <- deaths_lsoa_eng_asdr %>%
  left_join(msoa %>% select(LSOA11CD, MSOA11CD, LAD11CD), by = c("lsoa_code" = "LSOA11CD")) %>%
  rename(msoa_code = MSOA11CD, lad_code = LAD11CD) %>%
  select(lsoa_code, lsoa_name, msoa_code, lad_code, gender,
         population_lsoa, deaths_lsoa, stdrate) %>%
  distinct() %>%
  left_join(deaths_msoa_eng_asdr %>%
              select(msoa_code, gender, stdrate_msoa = stdrate, deaths_msoa),
            by = c("msoa_code","gender")) %>%
  left_join(deaths_lad_eng_asdr %>%
              select(lad_code, gender, stdrate_lad = stdrate, deaths_lad),
            by = c("lad_code","gender")) %>%
  group_by(gender) %>%
  mutate(
    stdrate_selected = case_when(
      deaths_lsoa < 5 ~ if_else(deaths_msoa < 10, stdrate_lad, stdrate_msoa), ##10 and 20 before this improves, still shoud be the same with calibration
      TRUE ~ stdrate
    ),
...
deaths_imd <- deaths_england %>%
  left_join(msoa, by = c("lsoa_code" = "LSOA11CD")) %>%
  left_join(deprivation, by = "lsoa_code") %>%
  group_by(imd_decile, gender, age) %>%
  summarise(deaths = sum(deaths, na.rm = TRUE), .groups = "drop")

population_imd <- population_england %>%
  left_join(msoa, by = c("lsoa_code" = "LSOA11CD")) %>%
  left_join(deprivation, by = "lsoa_code") %>%
  group_by(imd_decile, gender, age) %>%
  summarise(population = sum(population, na.rm = TRUE), .groups = "drop")

deaths_imd_eng <- deaths_imd %>%
  left_join(population_imd, by = c("imd_decile","gender","age")) %>%
  mutate(age_specific_rate = case_when(
    population > 0 ~ (deaths / population * 1000),
    population == 0 & deaths == 0 ~ 0,
    population == 0 & deaths  > 0 ~ NA_real_
...
deaths_imd_eng_asdr <- deaths_imd_eng %>%
  ...
  mutate(
    product = stdrate * pop,
    stdrate_ave = sum(product, na.rm = TRUE) / sum(pop, na.rm = TRUE),
    RR_imd = stdrate / stdrate_ave
  ) %>% ungroup()

england_lsoa_deaths <- england_lsoa_deaths %>%
  left_join(deprivation, by = "lsoa_code") %>%
  left_join(deaths_imd_eng_asdr %>% select(imd_decile, gender, RR_imd),
            by = c("imd_decile","gender"))
```
...
england_lifetable_lsoa <- england_lsoa_deaths %>% 
  ...
  select(lsoa_code, lsoa_name, gender = sex.x, age, rate1000, imd_decile, population_lsoa,RR)
...
lsoa11_to_lsoa21 <- readr::read_csv(lsoa11_to_lsoa21_path, show_col_types = FALSE) %>%
  dplyr::select(LSOA11CD, LSOA21CD, LAD22CD, LAD22NM)
...
england_lsoa21_syoa_cal <- england_lsoa21_syoa_cal %>% left_join(deprivation, by= c("lsoa21cd" = "lsoa_code")) %>% select(lsoa21cd, age, sex= gender, rate = rate_cal, imd_decile, deprivation, LAD22NM) %>%
  mutate(rate1000= rate*1000)

readr::write_csv(
  england_lsoa21_syoa_cal,
  here::here("manchester/health/processed/england_lsoa21_mxrates_calibrated.csv")
)

manchester_lsoa21_syoa_cal <- england_lsoa21_syoa_cal %>%
  dplyr::filter(LAD22NM %in% gm_lads)
...
data_plot <- england_lsoa21_syoa_cal %>%
  mutate(
...
p_manchester <- ggplot(data_plot, 
                    aes(x = age, y = rate1000,
                        ...
                        group = lsoa21cd)) +  # Each LSOA has its own line
  geom_line(lwd = 0.7, alpha = 0.5) +  # Reduce opacity to manage overplotting
  facet_wrap(~ LAD22NM) +  # Compare IMD deciles within each sex
  ylab("Mortality rate (per 1000)") + 
  xlab("Age (years)") +  
    scale_y_continuous(breaks=c(0.0002, 0.0005, 0.001, 0.005, 0.01, 0.05, 0.1, 0.2)*1000) +
  scale_color_viridis_d(option = "C", name = "IMD Decile") +  # Use discrete color scale
