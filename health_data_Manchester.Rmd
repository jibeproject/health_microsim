...
gbd_lads_small <- gbdp %>%
  ...
  mutate(small_lads = case_when(
    from_age > age_onset & val <= 20 ~ "yes",    
    TRUE ~ "no"                             
  ))
...
gbd_GM_small <- gbdp %>%
  ...
  ungroup() %>%
  left_join(disease_onset, by = "cause") %>%  # Explicitly join by cause
  mutate(small_gm = case_when(
    from_age > age_onset & val_GM <= 20 ~ "yes",    
    TRUE ~ "no"                             
  ), 
...
# #Repeat LAD values for LSOA (we are not doing this, we adjust the health code to use lads for diseases data)
# 
...
allcause<- allcause %>%
  ...
  mutate(sex=as.numeric(case_when(sex == "Males" ~ 1,
                                  sex=="Females" ~ 2))) %>%
  mutate(location = LSOA11CD) %>%
  select(age, sex, location, LSOA11CD, cause, rate, measure) %>%
...
diseases <- diseases %>% 
  ...
  mutate(sex=as.numeric(case_when(sex == "Male" ~ 1,
                                  sex=="Female" ~ 2))) %>%
  select(age, sex,location, cause, rate, measure) %>%
...
health_transitions_manchester <- bind_rows(allcause, incidence) %>% 
  left_join(lsoa_11to21 %>% 
            ...
            by = "LSOA11CD") %>% 
  select(lsoa21cd, LSOA11CD, age, sex, cause, rate, change11to21, location, location_type)
