...
england_lsoa_deaths <- deaths_lsoa_eng_asdr %>% 
  left_join(msoa, by = c("lsoa_code" = "lsoa21cd")) %>% 
  ...
  left_join(deaths_lad_eng_asdr %>% 
    select(lad_code, stdrate_lad = stdrate, gender),
            by = c("lad_code", "gender")) %>% 
  group_by(gender) %>% 
  mutate(
    # Adjust the logic to first check for deaths_lsoa and then deaths_msoa
    stdrate_selected = ifelse(
      deaths_lsoa == 0,            # If LSOA deaths are 0, use MSOA rate
      ifelse(deaths_msoa == 0,     # If MSOA deaths are 0, use LAD rate
             stdrate_lad,         
             stdrate_msoa),
