...
deaths_males <- read_xlsx('manchester/health/original/ons/DeathsbyLSOAmidyear11to21.xlsx', sheet = "2", skip = 2)
deaths_females <- read_xlsx('manchester/health/original/ons/DeathsbyLSOAmidyear11to21.xlsx', sheet = "3", skip = 2)
...
pop_males <- read_xlsx('manchester/health/original/ons/SAPE21DT1a-mid-2018-on-2019-LA-lsoa-syoa-estimates-formatted.xlsx', sheet = "Mid-2018 Males", skip = 3)
pop_females <- read_xlsx('manchester/health/original/ons/SAPE21DT1a-mid-2018-on-2019-LA-lsoa-syoa-estimates-formatted.xlsx', sheet = "Mid-2018 Females", skip = 3)
...
msoa <- fread(here('manchester/health/original/ons/lsoa_2011.csv'))
...
rr_plot
### Marina, we need to do something with the comparison with Europe. I think can be add on sensitivity analysis to the choice of standard population.
...
allcause_data <- allcause %>%
  ...
  mutate(sex=as.numeric(case_when(sex == "Males" ~ 1,
                                  sex=="Females" ~ 2))) %>%
  select(age, sex, location_code, location_type, cause, rate)
...
write.csv(health_transitions_manchester, here("manchester/health/processed/health_transitions_manchester.csv"))
```
