...
cause mortality is also dependent on diseases statues, meaning the if a
...
deaths in an age group in an lsoa, then a lad ASDRs should be used, and
...
#Repeat LAD values for LSOA

gbdpyr_interp_lsoa <- msoa %>%
  select(location = LAD11NM, LSOA11CD, LSOA11NM) %>% 
...
write_rds(gbdpyr_interp_lsoa, here("manchester/health/processed/manchester_diseases_lsoa_070325.RDS"))
...
diseases <- readRDS(here("manchester/health/processed/manchester_diseases_lsoa_070325.RDS"))
...
allcause<- allcause %>%
  ...
  mutate(sex=as.numeric(case_when(sex == "Males" ~ 1,
                                  sex=="Females" ~ 2))) %>%
  select(age, sex, LSOA11CD, cause, prob, measure)
...
health_transitions_manchester <- bind_rows(allcause_data, incidence) %>% 
  left_join(lsoa_11to21 %>% 
              select(LSOA11CD, lsoa21cd = LSOA21CD, change11to21 = CHGIND),
            by = "LSOA11CD") %>% 
  select(lsoa21cd, LSOA11CD, age, sex, cause, prob, change11to21)

# Save prevalence and incidence (includes deaths from all causes) separetly

write.csv(prevalence, here("manchester/health/processed/health_transitions_manchester_prevalence_070325.csv"))

write.csv(health_transitions_manchester, here("manchester/health/processed/health_transitions_manchester_raw_070325.csv"))
```
