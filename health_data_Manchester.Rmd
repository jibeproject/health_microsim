...
allcause <- read.csv(here("manchester/health/processed/manchester_lsoa21_mxrates_calibrated.csv"))
...
allcause<- allcause %>%
  ...
  mutate(sex=case_when(sex == "Males" ~ "male",
         ...
         age=as.numeric(age)) %>% 
  dplyr::select(age, sex, location_code, location_name, cause, rate, measure) %>%
