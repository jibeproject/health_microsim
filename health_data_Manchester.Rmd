...
6.  Adding Age-Standartised Mortality Rates from ONS as of February,
    ...
    TO DO: add new datasets Belen to list 
...
# Read the LSOA11 → LSOA21 lookup (with LAD22)

# LSOA11 -> LSOA21 -> LAD22 bridge  (no weights)
lsoa11_to_lsoa21_path <- here::here(
  "manchester","health","original","ons",
  "LSOA_(2012)_to_LSOA_(2021)_to_Local_Authority_District_(2022)_Best_Fit_Lookup_for_EW_(V2).csv"
)
stopifnot(file.exists(lsoa11_to_lsoa21_path))

lsoa11_to_lsoa21 <- readr::read_csv(lsoa11_to_lsoa21_path, show_col_types = FALSE) %>%
...
## Apply to life tables and calibrate
...
# Smooth, calibration results in some jumps (need to check again after smooting it matches observed)
...
# Save calibrated outputs with imd
...
#### Check with deaths counts per lad

library(dplyr)

# --- LAD names (as in your snippet)
lad_names <- oa21_lookup %>%
...
# --- Pick the smoothed rate, with safe fallbacks  ----------------
# Expecting: england_lsoa21_syoa_smoothed with columns lsoa21cd, (gender or sex), age,
#            and rate_smooth (preferred) or rate_cal or rate1000 (per 1000 persons).
...
p_manchester <- ggplot(data_plot, 
                    aes(x = age, y = rate1000,
                        ...
                        group = lsoa21cd)) +  # Each LSOA has its own line
  geom_line(lwd = 0.7, alpha = 0.5) +  # Reduce opacity to manage overplotting
  facet_wrap(~ LAD22NM, ncol = 4) +  # Compare IMD deciles within each sex
  ylab("Mortality rate (per 1000)") + 
  xlab("Age (years)") +  
    scale_y_continuous(breaks=c(0.0002, 0.0005, 0.001, 0.005, 0.01, 0.05, 0.1, 0.2)*1000) +
  scale_color_viridis_d(option = "C", name = "IMD Decile") +  # Use discrete color scale
...
# Comparison using English weights. Belen note (07/11/2025) after checking results using english weights and Euro weights, data aligns with published estimates with Euro weights, so using those, using english weights seems to push all rates down. 

# If you already computed English-weighted LAD ASDRs as `deaths_lad_eng_asdr` earlier:
...
deaths_lad_eng_asdr_manchester <- deaths_lad_asdr_compare %>% 
  filter(
...
deaths_lad_eu_asdr_manchester <- deaths_lad_eng_asdr %>% ### Rates used to derived ASMR are now using european weights
...
comparison_table <- nomis_lad %>% 
  left_join(deaths_lad_eng_asdr_manchester, by = c("gender","lad_name")) %>% 
  left_join(deaths_lad_eu_asdr_manchester,  by = c("gender","lad_name"))

print(comparison_table)

```

## Additional comparisons (Marina to add some explanation) (UP TO HERE)
...
write_csv(gbdp, here("manchester/health/processed/gbdp.csv"))
...
gbdp_nw <- gbd_england %>% filter(location == "North West England") %>%
  gbd_process(.) %>%
  filter(measure %in% c("Prevalence", "Incidence"))
write_csv(gbdp_nw, here("manchester/health/processed/gbdp_nw.csv"))

gbdp_england <- gbd_england %>% filter(location == "England") %>%
  gbd_process(.) %>%
  filter(measure %in% c("Prevalence", "Incidence"))
write_csv(gbd_england, here("manchester/health/processed/gbdp_england.csv"))
```
...
### Data for England and NorthWest is only for diseases identified as having small numbers for LADs and Greater Manchester (Breast cancer, Esophageal cancer, Stomach cancer, Nasopharynx cancer, Chronic myeloid leukemia, Larynx cancer, Other pharynx cancer, Multiple myeloma, Diabetes mellitus type 2, Lip and oral cavity cancer, Liver cancer, Bladder cancer)



# Flag LAD groups with any value < 20 from age 40+
lad_group_flags <- gbdp %>%
  group_by(measure, location, sex, agegroup, from_age, to_age, cause, metric, year, agediff) %>%
  summarise(replace_group = any(val < 20 & from_age >= 40), .groups = "drop")

# Attach replace flag to LAD-level data
gbd_lads_flagged <- gbdp %>%
  left_join(lad_group_flags, by = c("measure", "location", "sex", "agegroup", "from_age", "to_age", "cause", "metric", "year", "agediff"))

# Prepare fallback datasets

# GM fallback — aggregate LADs into GM
gbdp_GM <- gbdp %>%
  group_by(measure, sex, agegroup, from_age, to_age, cause, metric, year, agediff) %>%
  summarise(
    val_GM = sum(val),
  ...
  mutate(
    ...
    fallback_valid_GM = val_GM >= 20 & from_age >= 40
  )

#  NW fallback — already aggregated
gbdp_nw_clean <- gbdp_nw %>%
  rename(
    val_NW = val,
    val1yr_NW = val1yr,
    rate_1_NW = rate_1
  ...
  mutate(
    fallback_valid_NW = val_NW >= 20 & from_age >= 40
  )

# England fallback — already aggregated, always acceptable
gbdp_england_clean <- gbdp_england %>%
  rename(
    val_ENG = val,
    val1yr_ENG = val1yr,
    rate_1_ENG = rate_1
...
# Join fallback data to LAD-level observations
gbd_joined <- gbd_lads_flagged %>%
  rename(original_location = location) %>%
  left_join(gbdp_GM, by = c("measure", "sex", "agegroup", "from_age", "to_age", "cause", "metric", "year", "agediff")) %>%
  left_join(gbdp_nw_clean, by = c("measure", "sex", "agegroup", "from_age", "to_age", "cause", "metric", "year", "agediff")) %>%
  left_join(gbdp_england_clean, by = c("measure", "sex", "agegroup", "from_age", "to_age", "cause", "metric", "year", "agediff"))

#  Replace LAD values where flagged, based on fallback hierarchy
gbd_final <- gbd_joined %>%
  mutate(
    replaced_with = case_when(
      replace_group & fallback_valid_GM ~ "Greater Manchester",
      replace_group & !fallback_valid_GM & fallback_valid_NW ~ "North West",
      replace_group & (!fallback_valid_GM & !fallback_valid_NW) ~ "England",
      TRUE ~ "Original"
    ),
    val = case_when(
      replace_group & fallback_valid_GM ~ val_GM,
      replace_group & !fallback_valid_GM & fallback_valid_NW ~ val_NW,
      replace_group & (!fallback_valid_GM & !fallback_valid_NW) ~ val_ENG,
      TRUE ~ val
    ),
    val1yr = case_when(
      replace_group & fallback_valid_GM ~ val1yr_GM,
      replace_group & !fallback_valid_GM & fallback_valid_NW ~ val1yr_NW,
      replace_group & (!fallback_valid_GM & !fallback_valid_NW) ~ val1yr_ENG,
      TRUE ~ val1yr
    ),
    rate_1 = case_when(
      replace_group & fallback_valid_GM ~ rate_1_GM,
      replace_group & !fallback_valid_GM & fallback_valid_NW ~ rate_1_NW,
      replace_group & (!fallback_valid_GM & !fallback_valid_NW) ~ rate_1_ENG,
      TRUE ~ rate_1
    ),
    location = original_location
  ) %>%
  select(measure, sex, cause, year, agegroup, from_age, to_age,
         val, val1yr, rate_1, location, replaced_with, agediff)

# View how many groups were replaced
table(gbd_final$replaced_with)

gbdp <- gbd_final
write_csv(gbd_final, here("manchester/health/processed/gbd_processed.csv"))
