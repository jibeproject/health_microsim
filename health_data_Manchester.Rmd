...
prevalence and all cause mortality. A second iteration will add a
differential all cause mortality risk for individuals who have one or
more diseases. Last, we might want to include trends in mortality,
incidence of diseases and prevalence of diseases.
...
# Mortality and Poplation Data by LSOA by age and sex (2018) 
...
deaths_england <- fread(here('manchester/health/processed/ons/deaths_england.csv'))
...
population_england <- fread(here('manchester/health/processed/ons/population_england.csv'))
...
msoa <- fread(here('manchester/health/original/ons/PCD_OA21_LSOA21_MSOA21_LAD_AUG24_UK_LU.csv'))
...
lifetable_england  <- fread(here('manchester/health/original/ons/life_tables_20172019.csv'),skip = 5)
...
deprivation <- fread(here('manchester/health/original/ons/imd2019lsoa.csv')) 
...
european_pop <- fread(here('manchester/health/original/ons/european_standard_population_by_sex.csv'))
...
## Mortality by year of age and sex for England
...
then multiplying the result by 1000, then multiplying each of the
age-specific rates by the proportion of the population belonging to the
particular age group (standard population weight)
...
# Using ONS reported statistics for England population estimates (refs)
...
A relative mortality rate is produced for each area, defined as the rate
relative to the average mortality in England**.** This average mortality
is defined as a weighted average of the area-specific rates, weighted by
...
england_lsoa_deaths <- deaths_lsoa_eng_asdr |>
  ...
  select(lsoa_code, lsoa_name, msoa_code = "msoa21cd", lad_code = "ladcd",
         gender, population_lsoa, deaths_lsoa, stdrate) |>
  distinct() |>
  left_join(deaths_msoa_eng_asdr |> select(msoa_code, stdrate_msoa = stdrate, deaths_msoa, gender),
            by = c("msoa_code", "gender")) |>
  left_join(deaths_lad_eng_asdr |> select(lad_code, stdrate_lad = stdrate, gender),
            by = c("lad_code", "gender")) |>
  group_by(gender) |>
 mutate(stdrate_selected = ifelse( 
           is.na(stdrate) | deaths_lsoa < 20,   # if NA or fewer than 10 deaths at the lsoa level, use msoa
           ifelse(
             deaths_msoa > 40,             # if fewer than 20 deaths at the msoa level, use lad
             stdrate_msoa,
             stdrate_lad),
           stdrate),
        stdrate_ave = with(.data, sum(stdrate_selected*population_lsoa,na.rm = TRUE)/sum(population_lsoa))) |>                    # if not NA and more than or equal to 10, use lsoa
  mutate(RR = stdrate_selected / stdrate_ave) %>%
  ungroup()
```
...
area-specific, relative standardised rate.
...
england_lifetable_lsoa <- england_deaths_socio %>% 
  ...
  filter(sex.x == sex.y) %>% 
  select(lsoa_code, lsoa_name, sex = sex.x, age, rate1000, imd_decile, RR)
...
p_manchester <- ggplot(manchester_lifetable_lsoa, aes(x = age, y = rate1000)) + 
  geom_line(lwd=0.2) +   
...
## Alternative data set for mortality 

Data is available for standardised mortlaity rates for local authority
districts.
...
# Add LAD code
lad <- fread(here('manchester/health/original/ons/lsoa_to_msoa.csv'))
lad <- lad %>%
  select(6,7) %>%
  distinct()
...
saveRDS(gbdpyr_interp, here("manchester/health/processed/manchester_diseases_lad.RDS"))
