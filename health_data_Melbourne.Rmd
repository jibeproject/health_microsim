...
**Code if using ATLAS original up to years 2014 or 2016 (check)
(commented out as we can use latest data, code below)**
...
**Code if using latest ATLAS (2010-2019)**
...
search_string <- "invasive" #we just keep invasive cancer, stages are within invasive. in situ, localised, is not available as added to invasive. Most data for invasive and for previous ATLAS data we ony had invasive cancer. 
...
```{r}
# Stroke
stroke_prev <- bind_rows(
  # data by socioeconomic quintile
  read_xlsx(here("health/original/AIHW/cvd-diabetes-comorbidities.xlsx"),
  ...
  # add data for Australia all ages
  read_xlsx(here("health/original/AIHW/cvd-diabetes-comorbidities.xlsx"), 
            sheet = "Stroke", skip = 8, col_names = TRUE, n_max = 9) %>%
    slice(-c(1:8)) %>%
    rename_with( ~ c("socio", "MaleNo", "FemaleNo", "PersonNo", "blank1",
                     "Male", "MaleCI", "Female", "FemaleCI", "Person", "PersonCI")) %>%
    dplyr::select(socio, Male, Female) %>%
  ...
  mutate(socio = case_when(socio=="Group 1 (lowest)" ~ "1", # 1: lowest, to 5: highest
                           ...
                           socio=="Group 4" ~ "4",
                           socio=="Group 5 (highest)" ~ "5",
                           socio == "Age-standardised rate(a)" ~ "6",
                           .default = socio),
         socio = as.numeric(socio)) %>%
  arrange(socio) %>%
  group_by(sex) %>%
...
chd_prev <- bind_rows(
  ...
  # add data for Australia all ages
  read_xlsx(here("health/original/AIHW/cvd-diabetes-comorbidities.xlsx"), 
            sheet = "CHD", skip = 8, col_names = TRUE, n_max = 7) %>%
    slice(-c(1:6)) %>%
    rename_with( ~ c("socio", "MaleNo", "FemaleNo", "PersonNo", "blank1",
                     "Male", "MaleCI", "Female", "FemaleCI", "Person", "PersonCI")) %>%
    dplyr::select(socio, Male, Female) %>%
