...
PREPARING TABLE IN EXCEL (disease_data, Melbourne final)
...
output_dir <- "images/inc_gbd_age_sex/"
...
AIHW, initial code from Chris used AIHW so we still harmonise, also to
match standard names used across different inputs (i.e. health data,
dose response functions). We also use the table for other purposes (TO
COMPLETE, USED SOMEWHERE IN PREPARATIONADN INTERPOLATION OF THE DATA).
...
# Generate and save all pages
output_dir <- "images/inc_aihw_age_sex/"
if (!dir.exists(output_dir)) {
  dir.create(output_dir, recursive = TRUE)
}
...
pop_SA2 <- read_csv("health/original/ABS/Population by SA2 area.csv", skip = 9, col_names = TRUE) %>%
...
#Up to here, calculate weighted average with populaiton
compare_socio <- cancer_incidence_area %>%
  ...
  group_by(age, sex, site, socio, rate_ave_100000, .groups = "drop") %>%
  summarise(
...
compare_age_sex <- cancer_incidence_area %>%
  ...
  group_by(age, sex, site, rate_ave_100000, .groups = "drop") %>%
  summarise(
    # Only calculate the weighted mean if both rate_area_100000 and population are available
...
## Prepared by chris using AIHW data by socioeconomic quintile. 
...
compare_socio <- compare_socio %>% left_join(cancerinc, by=c("site", "age", "sex", "socio")) %>%
  mutate(rate_soc=round(rate, 2)) %>% select(!rate) %>% left_join(compare_age_sex)
...
BZD: Thank you Steve, I have changed the function, to take the weighted
average by population size to calculate the mean by SES for areas. Each
area would contribute to the mean differently by index of disadvantage
differently.

-   **breast cancer (F) and prostate cancer (M) show the reverse
    pattern, with most disadvantaged having lowest incidence.**

    BZD: this patters in confirmed with the source data saved in
    cancer_socioeonomic for incidence. For mortality, the most deprived
    die at a higher rate. In our model we will show change in incidence,
    and then these people become prevalent cases, but then those of
    lower SES die at a higher rate. We need, if possible, to bring
    prevalence into the story to see who are the prevalent cases after
    10 or 5 years of diagnosis. We need to think about how to calibrate
    the model and what data for it. If we do not account for prevalence
    and deaths by cause, then, we might need to limit the reporting to
    incidence (i.e. how many cases of diseases could be prevented to
    happen in the first place).

-   **gastric cancer (F&M) have all groups being below average:**

    The population weighted estimates for males are now a little bit
    more aligned, with most advantaged below average, advantage aligned
    with average, middle SES and disadvantaged a bit below and most
    disadvantaged a little bit above. For females it improves a bit,
    with advantaged group now in line with average. Number for this
    cancer is small, so the estimates for the areas would be highly
    uncertain based on low observations for SA2, so a bit uncertain.

-   **liver cancer (F) has all groups below average.**

    It improved a bit, but still some inconsistencies. The rates for
    females are a bit lower than males, there is probably some
    uncertainty in the SA estimates. But still inconsistent with
    cancer_socioeconomic, which shows, for the ASR a consistent gradient
    for both males and females with decreasing rates as socio economic
    status increases.

-   **malignant melanoma (F&M) show an inconsistent pattern with most
    results above average.**

    Weighted estimates change, but not in the same direction as in the
    cancer_socioeconomic, where the most advantage have the highest rate
    of incidence (for both males and females combined).

-   **myeloid leukemia (F&M) show little differentiation between socio
    economic groups.**

    Figures are very small for this cancer.

-   **myeloma (F&M) have most groups at or below average.**

Figures are also very small for this cancer.
...
**BZD: need to calculate back to average from areas, and compare with
average by age and sex. This is now done, below, small cancers show the
...
folder_path <- file.path("images", "inc_cancer_compare")
...
***Graphs comparing, for each disease and sex, rates for Australian
average from AIHW and by age and sex from data derived using data from
ATLAS aggregated by socioeconomic group.***
...
# Create folder if it doesn't exist
folder_path <- file.path("images", "inc_cancer_compare_ave")
if (!file.exists(folder_path)) {
  dir.create(folder_path, recursive = TRUE)
}

getwd# Define colors for rate types
rate_type_colors <- c("black", "red", "blue")
...
```{r}
age_standirdised_rate_soc <- cancer_incidence_area %>%
  # Filter out rows where population is NA to avoid issues with the weighted mean
  filter(!is.na(population), !is.na(rate_area_100000)) %>%
  
  group_by(site, socio, .groups = "drop") %>%
  summarise(
    # Only calculate the weighted mean if both rate_area_100000 and population are available
   ASR = sum(rate_area_100000*(population/sum(population))),
    .groups = "drop"
  )  %>%
  mutate(socio = as.factor(socio)) %>%
  filter(socio != "Not classified") %>%
...
**STEVE, YOU MIGHT HAVE NOTED THAT i REMOVE ESTIMATES FROM CHRIS, WOULD
YOU BE ABLE TO INTERPOLATE HIS ESTIMATES AND ADD BACK PLEASE? THE LACK
ON INTERPOLATION MESSES UP THE GRAPH.**

**THIS NEEDS REWRITING BASED ON OUR NOTES AND ALL THREE COMPARISON:
ATLAS AVERAGE BY SOCIOEOCNOMIC VS AUS AVERAGE, ATLAS AVERAGE AGE AND SEX
VS AUS AVERAGE AGE AND SEX, ATLAS AVERAGE BY SOCIOECONOMIC VS CHRIS
AVERAGE BY SOCIOECONOMIC.**
...
age, sex and SES quintile. **STEVE THIS IS POSSIBLE, WOULD BE FOR
AUSTRALIA WIDE, AND THESE DATA WOULD BE AVAILABLE FROM TABLE BUILDER.**
...
# save plot
# Generate and save all pages
output_dir <- "images/inc_cvd_dia_copd/"
if (!dir.exists(output_dir)) {
  dir.create(output_dir, recursive = TRUE)
}
...
**STEVE, COULD YOU DO DEMENTIAS AS WELL, I SEE THAT WE HAVE
HOSPITALIATIONS ADN MORTLAITY, IS THERE MUCH DIFFERENCE IN THE DERIVE
RRS?**

**I BELIEVE TAHT WE DO NOT HAVE DATA FOR DEPRESSION FOR SES.**
...
```{r}
# GBD data for Melbourne. 
gbd <- read.csv("docs/health/original/GBD/gbd_prevalence.csv")
...
saveRDS(cancer_prevalence_area, file="docs/health/processed/cancer_prevalence_area.rds")
...
```{r}
# Stroke
stroke_prev <- read_xlsx("docs/health/original/AIHW/cvd-diabetes-comorbidities.xlsx", sheet = "Stroke", 
...
chd_prev <- read_xlsx("docs/health/original/AIHW/cvd-diabetes-comorbidities.xlsx", sheet = "CHD", 
...
saveRDS(cvd_dia_copd_prev, file="docs/health/processed/cvd_dia_copd_prevalence.rds")
```
