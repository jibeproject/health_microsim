...
health_outcomes_risks <- tibble::tribble(
  ...
  "Parkinson disease",  "GBD (G20)", "AIHW (G20)","PA", "#", "#", "#",
)
...
gbd <- bind_rows(
  read.csv(here("melbourne/preprocessing/health/original/GBD/gbd_incidence_2018_2019.csv")),
  read.csv(here("melbourne/preprocessing/health/original/GBD/gbd_incidence_2020_2021.csv")),
  read.csv(here("melbourne/preprocessing/health/original/GBD/gbd_prevalence_2018_2019.csv")),
  read.csv(here("melbourne/preprocessing/health/original/GBD/gbd_prevalence_2020_2021.csv")))
...
# Parkinson's disease prevalence (GBD age/sex only; no SES disaggregation available)
# Parkinson's was not included in the main GBD download, so it is loaded from a
# separately downloaded GBD 2021 file and processed with the identical pipeline.
parkinson_gbd_raw <- read.csv(
  here("melbourne/health/original/GBD/Parkinsons/2018/IHME-GBD_2021_DATA-933bcb4f-1.csv")
)

# GBD 2021 downloads use 'measure_name', 'sex_name', etc. Rename to match the
# column names used by the rest of the pipeline.
parkinson_gbd_raw <- parkinson_gbd_raw %>%
...
# Apply the same pre-processing as the main gbdp block.
# Keep BOTH Incidence and Prevalence: prevalence feeds baseline disease
# assignment (SM_prev) and incidence feeds the SILO transitions file (SM).
parkinson_gbdp <- parkinson_gbd_raw %>%
  filter(metric %in% "Rate", measure %in% c("Incidence", "Prevalence")) %>%
  select(-any_of(c("location", "upper", "lower",
  ...
  mutate(
    from_age = case_when(age == "95+" ~ 95L, age == "<5" ~ 0L, TRUE ~ from_age),
    to_age   = case_when(age == "95+" ~ 99L, age == "<5" ~ 4L, TRUE ~ to_age),
    agediff  = to_age - from_age + 1,
    val1yr   = rate_1
  ) %>%
  rename(agegroup = age)

# Stretched 5-year-to-1-year frame (used for validation join after interpolation)
parkinson_gbdpyrd5 <- parkinson_gbdp[
    rep(seq_len(nrow(parkinson_gbdp)), parkinson_gbdp$agediff), ] %>%
  mutate(ageyr = from_age + sequence(parkinson_gbdp$agediff) - 1) %>%
  select(measure, ageyr, sex, agegroup, from_age, to_age, cause, year, val1yr, rate_1)

# Interpolate from 5-year age groups to single years using the same function
# (disagg_smooth_spline) as all other GBD diseases
parkinson_grp <- parkinson_gbdp %>%
  group_by(measure, year, sex, cause) %>%
  arrange(measure, year, sex, cause, from_age)

parkinson_interp <- group_modify(parkinson_grp, disagg_smooth_spline) %>%
  ungroup() %>%
  left_join(parkinson_gbdpyrd5, by = c("measure", "year", "ageyr", "sex", "cause"))

# Extract 2018 prevalence and compute annual probability
parkinson_prevalence <- parkinson_interp %>%
  filter(measure == "Prevalence" & year == 2018) %>%
  rename(rate = val_interpolated) %>%
  mutate(prob = 1 - exp(-rate))

cat("parkinson_prevalence: rows =", nrow(parkinson_prevalence),
    "| ages =", n_distinct(parkinson_prevalence$ageyr),
    "| sexes =", paste(sort(unique(parkinson_prevalence$sex)), collapse = ", "), "\n")

# Extract 2018 incidence (used for the SILO transitions file, SM). `rate` is the
# annual incidence rate; SILO applies 1 - exp(-rate) internally (see SM block).
parkinson_incidence <- parkinson_interp %>%
  filter(measure == "Incidence" & year == 2018) %>%
  rename(rate = val_interpolated) %>%
  mutate(prob = 1 - exp(-rate))

cat("parkinson_incidence: rows =", nrow(parkinson_incidence),
    "| ages =", n_distinct(parkinson_incidence$ageyr),
    "| sexes =", paste(sort(unique(parkinson_incidence$sex)), collapse = ", "), "\n")
...
## IRSD decile lookup for Greater Melbourne SA2s

The ABS SEIFA file (catalogue 2033.0.55.001) contains the Index of Relative
Socio-Economic Disadvantage (IRSD) score and national decile for each SA2.
We compute Melbourne-specific decile ranks (i.e. relative disadvantage within
the study area, analogous to Manchester's IMD deciles) in addition to retaining
the national decile for comparability.

This lookup (`SA2_IRSD`) is used to replace the coarse quintile `socio`
variable with a 10-group decile in the health transitions prevalence output
(`SM_prev`). Note: for conditions where only quintile-level relative-risk data
are available (AIHW sources), deciles are mapped back to their parent quintile
for the disaggregation step — upgrading to true decile-level RRs requires new
data acquisition.

```{r}

# Load IRSD scores from ABS SEIFA 2016 SA2 file
# Source: ABS Catalogue 2033.0.55.001, Table 1 (SA2 IRSD)
# if the file structure differs.
irsd_raw <- read_xls(
  here("melbourne/health/original/ABS/2033055001 - sa2 indexes.xls"),
  sheet = "Table 2",
  skip  = 5
)

# Identify and rename the key columns: SA2 code, IRSD score, national decile
irsd_raw <- irsd_raw %>%
  rename_with(~c("sa2_code_str", "sa2_name", "sa2_population", "irsd_score", "irsd_rank", "irsd_decile_nat", "irsd_percentile_nat"),
              .cols = c(1:4, 6:8)) %>%
  mutate(sa2_code = suppressWarnings(as.numeric(sa2_code_str))) %>%
  filter(!is.na(sa2_code)) %>% 
  select(sa2_code, sa2_name, irsd_score, irsd_decile_nat) 

# Build Melbourne-specific IRSD decile (10 groups, equal count within Melbourne)
SA2_IRSD <- irsd_raw %>%
  filter(sa2_code %in% SA2_Melb$SA2_MAINCODE_2016) %>%
  dplyr::select(sa2_code, sa2_name, irsd_score, irsd_decile_nat) %>%
  arrange(irsd_score) %>%
  mutate(
    # Melbourne-local decile (1 = most disadvantaged within Melbourne)
    irsd_decile_mel = ntile(irsd_score, 10),
    # For joining to quintile-level RR data: map decile pairs → quintile
    # decile 1-2 → quintile 1, 3-4 → 2, 5-6 → 3, 7-8 → 4, 9-10 → 5
    irsd_quintile_mel = ceiling(irsd_decile_mel / 2)
  )

# Save for downstream use
saveRDS(SA2_IRSD, file = here("melbourne/health/processed/sa2_irsd_deciles.rds"))

cat("IRSD decile lookup built for", nrow(SA2_IRSD), "SA2s in Greater Melbourne.\n")
cat("Melbourne-local decile distribution:\n")
print(table(SA2_IRSD$irsd_decile_mel))
...
SILO/MITO code requires all-cause mortality and disease incidence,
disaggregated by age, sex and SA2 (and no other disaggregation),
for age 0 to 100.

The value supplied is an annual **rate** (per person-year). The SILO
Melbourne model (`DiseaseModelMEL`) reads this column (still named `prob`
for backwards-compatibility with `HealthTransitionTableReaderMEL`),
multiplies it by the relevant exposure/behaviour relative risks, and then
converts it to an annual probability internally via `1 - exp(-rate)`
before applying the survival function. Supplying a rate (rather than a
pre-converted probability) avoids a double `1 - exp(-.)` conversion and
is important for mortality at older ages, where `qx` is large.
...
SM_mort <- deaths_age_sa2 %>%
  ...
  # rate1000 is per-1000 qx ("the proportion of persons dying between exact age
  # x and exact age x+1"), i.e. an annual probability. SILO treats the value it
  # reads as a *rate* and applies 1 - exp(-rate) itself, so convert qx -> rate
  # here: rate = -log(1 - qx). Guard against qx = 1 at the open-ended oldest age
  # (which would give an infinite rate).
  mutate(qx = pmin(rate1000 / 1000, 1 - 1e-9),
         prob = -log(1 - qx), # 'prob' column carries a rate; see note above
         sex = case_when(
           sex == "Male" ~ 1,
           sex == "Female" ~ 2
         ),
...
# Carry the raw annual incidence `rate` in the `prob` column (SILO applies
# 1 - exp(-rate) itself; see note above). At incidence magnitudes rate ~= prob,
# but using the rate keeps the semantics consistent with mortality.
SM_inc_base <- data_melbourne %>%
  ...
  dplyr::select(SA2_MAIN16 = sa2_code, age, sex, cause, prob = rate, socio)
...
SA2_SES <- data_melbourne %>%
  ...
  ungroup() %>%
  # Join IRSD decile lookup so SA2_SES carries both the derived quintile (for
  # backwards-compatibility) and the ABS-based IRSD decile and Melbourne-local decile
  left_join(SA2_IRSD %>%
              dplyr::select(sa2_code, irsd_decile_nat, irsd_decile_mel,
                            irsd_quintile_mel),
            by = "sa2_code")
...
# parkinson - like depression, no SES disaggregation, so same value for all SA2s.
# (parkinson_incidence carries the annual rate; kept in the `prob` column.)
parkinson_inc_base <- parkinson_incidence %>%
  mutate(sex = case_when(sex == "Female" ~ 2, sex == "Male" ~ 1),
         cause = "parkinson") %>%
  dplyr::select(age = ageyr, sex, cause, prob = rate)

# extend the oldest available age out to 100 (as done for the other causes)
parkinson_inc_oldage <- parkinson_inc_base %>%
  group_by(sex, cause) %>%
  filter(age == max(age)) %>%
  ungroup() %>%
  filter(age < 100) %>%
...
SM_inc_parkinson <- bind_rows(parkinson_inc_base, parkinson_inc_oldage) %>%
  rowwise() %>%
  mutate(SA2_MAIN16 = list(SA2_SES$sa2_code)) %>%
  unnest(SA2_MAIN16) %>%
  ungroup()
...
# Diseases modelled by SILO: those with at least one exposure dose-response
# relationship in disease_outcomes_lookup.csv (any of air_pollution_pm25,
# air_pollution_no2, physical_activity, noise or ndvi = 1). This is the single
# source of truth for disease scope and matches the Java `Diseases` enum
# (silo/extensions/health/.../disease/Diseases.java) and the `disease_cols`
# list in assign_prevalence_Melbourne.R. Excluded: aggregates (all_cause_cancer)
# and diseases with no dose-response (kidney_cancer, malignant_melanoma,
# prostate_cancer, rectum_cancer - the latter is covered by colon_cancer, which
# represents "colon and rectum cancer"). all_cause_mortality is added for SM only.
modelled_diseases <- c(
  "all_cause_dementia", "bladder_cancer", "breast_cancer", "colon_cancer",
  "copd", "coronary_heart_disease", "depression", "diabetes",
  "endometrial_cancer", "esophageal_cancer", "gastric_cardia_cancer",
  "head_neck_cancer", "liver_cancer", "lung_cancer", "myeloid_leukemia",
  "myeloma", "parkinson", "stroke"
)
...
SM <-  bind_rows(SM_mort, SM_inc_cancer, SM_inc_non_cancer, SM_inc_depression,
  ...
  # keep only causes the model can use (modelled diseases + all-cause mortality)
  filter(cause %in% c("all_cause_mortality", modelled_diseases))

cat("SM: rows =", nrow(SM), "| causes =", paste(sort(unique(SM$cause)), collapse = ", "), "\n")
...
## Prevalence: SA2-level output for baseline disease assignment

Analogous to the `SM` incidence output above, but using **prevalence** probabilities.
This is used by `assign_prevalence_Melbourne.R` to assign baseline disease status to the
synthetic population. Using incidence rates for this purpose would massively undercount
chronic diseases (annual incidence << period prevalence for COPD, diabetes, etc.).

```{r}

# --- Cancers: cancer_prevalence_area has 5-digit Atlas SA2 codes; convert to
# 9-digit SA2_MAINCODE_2016 to match the synthetic population's SA2_MAIN16. ---
SM_prev_cancer <- cancer_prevalence_area %>%
  left_join(SA2_Melb %>% dplyr::select(SA2_5DIGITCODE_2016, SA2_MAINCODE_2016),
            by = c("sa2_code" = "SA2_5DIGITCODE_2016")) %>%
  mutate(sex = case_when(sex == "Male" ~ 1, sex == "Female" ~ 2)) %>%
  dplyr::select(SA2_MAIN16 = SA2_MAINCODE_2016, age, sex, cause, prob) %>%
  filter(!is.na(SA2_MAIN16))

# --- CVD/diabetes/COPD/dementia/stroke: map socio quintile → SA2 via SA2_SES ---
# Uses `irsd_quintile_mel` (Melbourne-local IRSD quintile derived from SA2_IRSD) so
# that the quintile assignment is based on the actual ABS IRSD score rather than
# the coarse `socio` column inherited from the disease-rate data.
# The 4 industrial SA2s (no IRSD data) fall back to middle quintile 3.
SM_prev_non_cancer <- cvd_dem_dia_copd_prev %>%
  filter(!is.na(socio)) %>%
  mutate(sex = case_when(sex == "Male" ~ 1, sex == "Female" ~ 2)) %>%
  dplyr::select(socio, cause, age, sex, prob) %>%
  full_join(
    SA2_SES %>% mutate(socio = ifelse(is.na(irsd_quintile_mel), 3, irsd_quintile_mel)) %>%
      dplyr::select(sa2_code, socio),
    by = "socio", relationship = "many-to-many"
  ) %>%
  mutate(SA2_MAIN16 = sa2_code) %>%
  dplyr::select(SA2_MAIN16, age, sex, cause, prob)

# --- Depression: no SES disaggregation; same probability for all SA2s ---
# depression_prevalence already has prob = 1 - exp(-rate) from its creation block
SM_prev_depression <- depression_prevalence %>%
  mutate(sex = case_when(sex == "Female" ~ 2, sex == "Male" ~ 1),
         cause = "depression") %>%
  dplyr::select(age = ageyr, sex, cause, prob) %>%
  rowwise() %>%
  mutate(SA2_MAIN16 = list(SA2_SES$sa2_code)) %>%
  unnest(SA2_MAIN16) %>%
  ungroup()

# --- Parkinson's: no SES disaggregation; same probability for all SA2s ---
SM_prev_parkinson <- parkinson_prevalence %>%
  mutate(sex = case_when(sex == "Female" ~ 2, sex == "Male" ~ 1),
         cause = "parkinson") %>%
  dplyr::select(age = ageyr, sex, cause, prob) %>%
  rowwise() %>%
...
# --- Combine and standardise cause names to lowercase snake_case ---
# Each component block keeps its original cause names; standardisation is done
# here so the output matches the disease_cols list in assign_prevalence_Melbourne.R.
SM_prev <- bind_rows(SM_prev_cancer, SM_prev_non_cancer, SM_prev_depression,
                     SM_prev_parkinson) %>%
  dplyr::select(SA2_MAIN16, age, sex, cause, prob) %>%
  mutate(
    cause = tolower(cause),
    cause = gsub(" ", "_", cause),
    cause = case_when(
      cause == "chronic_obstructive_pulmonary_disease"  ~ "copd",
      cause == "diabetes_type_2"                        ~ "diabetes",
      cause == "diabetes_mellitus_type_2"                ~ "diabetes",
      cause == "alzheimer's_disease_and_other_dementias" ~ "all_cause_dementia",
      cause == "head_and_neck_cancer"                   ~ "head_neck_cancer",
      cause == "ischemic_heart_disease"                 ~ "coronary_heart_disease",
      TRUE                                              ~ cause
    )
  ) %>%
  filter(!is.na(SA2_MAIN16), !is.na(prob)) %>%
  # keep only modelled diseases, so baseline prevalence matches the model scope
  # (SILO silently drops any disease not in its `Diseases` enum). If non-modelled
  # cancers are wanted purely for descriptive validation, relax this filter.
  filter(cause %in% modelled_diseases)

cat("SM_prev: rows =", nrow(SM_prev), "| unique causes =", n_distinct(SM_prev$cause), "\n")
cat("Causes present:\n")
print(sort(unique(SM_prev$cause)))

# write prevalence output for use in assign_prevalence_Melbourne.R
write_csv(SM_prev,
          file = here("melbourne/health/processed/health_transitions_melbourne_prevalence_SA2.csv"))
...
Prevalence rates for 2018 and subsequent years. 
...
dementia and COPD. 
<!-- SP note: the COPD figures from AIHW are much
