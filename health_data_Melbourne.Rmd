...
gbdp <- gbd %>%
  ...
  filter(year %in% 2018) %>%
  select(-c(location, upper, lower)) %>%
  filter(cause %in% "Depressive disorders") %>%
  mutate(rate_1=val/100000) %>% 
  # some ages have 'years' (eg 5-9 years), while others don't (eg 80-84); omit 'years'
  mutate(age = gsub(" years", "", age)) %>%
  tidyr::extract(age, c("from_age", "to_age"), "(.+)-(.+)", remove=FALSE, convert=TRUE) %>%
...
#Join with original data where rates are the same within groups to validate interpolated data
depression_incidence <- gbdpyr %>%
  left_join(gbdpyrd5, by = c("measure", "ageyr", "sex", "cause")) %>%
  rename(age = ageyr) %>%
  mutate(prob=1-exp(-rate_1)) %>%
  select(c("age", "sex", "measure", "cause", "prob")) %>%
...
TO DO: Change path of data when moved to original/ABS

```{r}
library(readxl)

# Read the Excel sheet and skip the first 6 rows
mort_trends <- read_excel(here("health/original/staged_projections and trends TEMPORARY FOLDER/32220ds02_2022-2071_projection_assumptions_detailed.xlsx"), 
...
mort_trends_ord[1, "diff"] <- 0 # modify female diff for 0 age 2022 as it compare with females. 
