...
parkinson_prevalence <- gbdpyr_interp %>%
  filter(measure == "Prevalence" & cause == "Parkinson's disease" & year == 2018) %>%
  rename(rate = val_interpolated) %>%
  mutate(prob = 1 - exp(-rate))
...
SA2_SES <- data_melbourne %>%
  ...
  left_join(SA2_IRSD %>% dplyr::select(sa2_code, irsd_decile_nat, irsd_decile_mel,
                                        irsd_quintile_mel),
            by = "sa2_code")
...
# --- Cancers: cancer_prevalence_area already has SA2-level prevalence probs ---
...
# --- Combine and standardise cause names (match SILO/MITO Diseases.java) ---
