...
mort_disagg <- mort_disagg |> 
  ...
  mutate(disag_educ = ifelse(between(age, 25, 84) & 
         ...
         prob = 1 - exp(-rate/1000)) |>  ## SP: is this right? The 'rates' in the life
      # tables are 'qx' - isn't that actually a proportion/probability, not a continuous rate?
  filter(sa2_code %in% sa2$SA2_MAINCODE_2016[sa2$GCCSA_NAME_2016=="Greater Melbourne"])
...
-   `mort_base.rds`: mortality rates for the base year of 2018 by age and sex.
-   `[filename]` : \[mortality trend factors for subsequent years\].
-   `mort_disagg_rates.rds`: relative rates to apply to the mortality rates to disaggregate by SA2 and education.

**SP note: Currently this only combines the mortality rates and disaggregating factors for 2018.  This section needs to be expanded so it also picks up subsequent years (work in progress).**
...
mort <- mort_base %>%
  # agegroup column to match the groups in mort_disagg_rates
  mutate(agegroup = cut(age, c(0, 25, 45, 65, 85, Inf),
                        right=FALSE, include.lowest = TRUE)) %>%
  # join the disagg_rates (disaggregated for SA2 and education)
  left_join(mort_disagg_factors, by = c("agegroup"),
            relationship = "many-to-many") %>%
  # calculate rate (per 1000, and disaggregated) and prob 
  mutate(rate = rate * 1000 * disagg_factor,
         prob = 1 - exp(-rate/1000)) %>% ## SP: is this right? The 'rates' in the life tables
      # are 'qx' - isn't that actually a proportion/probability, not a continuous rate?
  # select required fields
  dplyr::select(age, sex, sa2_code, sa2_name, educ, rate, prob)

# checking that the results are the same as 'mort_disag' - they are - will delete this check
chk <- mort_disagg %>% 
  left_join(mort, by = c("age", "sex", "sa2_code", "sa2_name", "educ")) %>%
  mutate(rate.diff = rate.x - rate.y,
         prob.diff = prob.x - prob.y)
