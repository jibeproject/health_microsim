...
# This value ranges from 0 (low) to 1 (high) and represents the probabillity that  # the area's estimate is different to the Australian average. 
...
# If the excess hazard ratio (EHR) is below 1, and the probability is high (>0.6),# # then it is likely this area really has a lower excess death rate than the # national average (i.e. higher survival). 
...
# BZD: Steve, yes, can you please copy the code back to generate the data as per Chris' code? then we do not need to be getting the data from elsewhere. Also, can you please interpolate? otherwise difficult to compare with ATLAS generated data and average by age and sex. Once the data is interpolated can you please add it back to the comparison graphs. Thank you. 
...
cancerinc <- readRDS(here("health/processed/cancer_incidence.rds"))
...
case. We have source data from IIHW to check on patterns
(health/original/AIHW/cancer_socioeconomic)
...
**BZD: Steve, can you please add back comparison with Chris' data? thank
you. the file cancer_incidence.RDM is in archive folder.**
...
The patterns of values is in the same direction, but aggregated values
from ATLAS generated data, aggregated by diseases are higher that those
in cancer_socioecononmic. ATLAS data aggregates, depending on cancers,
for the years range 2010-2014.
...
### BZD: Steve, I am thinking that we should use mortality, prevalence is self reported, from the health survey, whereas mortality is from death certificates. Also for mortality we have age standardises rates per 100,000 which is better to use and derive the RR as I modified for the other diseases relative to the Aus average. The dataset copd-mortality has in table Resp_Mort.1 standardised rates for age groups and males and females, since the rates for socioeconomic in table Table REsp_Mort2 are by sex, you could compare the standardised rates, so as per cvds, rate 6 is for australia wide (from table 1 here)
copd_inc <- read_xlsx(here("health/original/AIHW/copd-prevalence.xlsx"), sheet="Table 1.2", skip=2, 
...
inc_cvd_dia_copd_socio <- bind_rows(cvd_inc, diabetes_inc, stroke_inc, copd_inc) %>% arrange(cause, sex, socio) %>% 
  filter(socio != "6")
```
...
# save plot
# Generate and save all pages
output_dir <- file.path(here("images/inc_cvd_dia_copd/"))
if (!dir.exists(output_dir)) {
  dir.create(output_dir, recursive = TRUE)
}
...
cvd_dia_copd_inc <- cvd_dia_copd_inc %>% select("socio", "cause", "age", "sex", "prob") %>% mutate(measure="incidence")
saveRDS(cvd_dia_copd_inc, file=(here("health/processed/cvd_dia_copd_incidence_final.rds")))
```

**STEVE, COULD YOU DO DEMENTIAS AS WELL, I SEE THAT WE HAVE
HOSPITALIATIONS AND MORTLAITY, IS THERE MUCH DIFFERENCE IN THE DERIVE
RRS? As with corrections, we need to derive the RRs from the Australian
average.**
...
# save plot
# Generate and save all pages
output_dir <- file.name(here("images/prev_cvd_dia_copd/"))
if (!dir.exists(output_dir)) {
  dir.create(output_dir, recursive = TRUE)
}
