...
gbd <- bind_rows(
  read.csv(here("health/original/GBD/gbd_incidence_2018_2019.csv")),
  read.csv(here("health/original/GBD/gbd_incidence_2020_2021.csv")),
  read.csv(here("health/original/GBD/gbd_prevalence_2018_2019.csv")),
  read.csv(here("health/original/GBD/gbd_prevalence_2020_2021.csv")))
...
```{r}
# Get sheet with SA2 areas by SES and filter Victoria [SP: currently the filter for Victoria is commented out; is that intentional?]

library(stringr)  # SP: can delete; loaded in chunk 1 as part of tidyverse
library(openxlsx)  # SP: can delete; I've added it to chunk 1 

excel_file <- loadWorkbook(here("health/original/ATLAS 2/ACA 2 - Diagnosis relative estimates.xlsx"))
...
saveWorkbook(excel_file, here("health/original/ATLAS 2/ATLAS_clean.xlsx"), overwrite = TRUE)


# Open file 
excel_file <- here("health/original/ATLAS 2/ATLAS_clean.xlsx")
...
copd_inc <- read_xlsx(here("health/original/AIHW/copd-mortality.xlsx"),
...
```{r}
# Stroke
stroke_prev <- read_xlsx(here("health/original/AIHW/cvd-diabetes-comorbidities.xlsx"),
...
chd_prev <- read_xlsx(here("health/original/AIHW/cvd-diabetes-comorbidities.xlsx"),
...
saveRDS(cvd_dem_dia_copd_prev, file=here("health/processed/cvd_dem_dia_copd_prevalence.rds"))
```
...
data_melbourne <- mort_disagg %>% bind_rows(cancer_incidence_area) %>%
                  bind_rows(cancer_prevalence_area) %>%
                  bind_rows(cvd_dem_dia_copd_inc) %>%
                  bind_rows(cvd_dem_dia_copd_prev) %>%
                  bind_rows(depression_incidence) %>%
                  bind_rows(depression_prevalence) %>%
                  mutate(cause=tolower(cause), 
                         measure=tolower(measure)) %>%
                  select(age, sex, sa2_code, sa2_name, educ, prob, cause, measure, socio)

saveRDS(data_melbourne, file=here("health/processed/health_transitions_melbourne.rds"))
```
...
mort_trends <- read_excel(here("health/original/ABS/32220ds02_2022-2071_projection_assumptions_detailed.xlsx"), 
...
Regression analysis to derive trends

```{r}

# Load necessary libraries - TO DO - delete (because both are in tidyverse, loaded at the top) 
library(dplyr)
library(ggplot2)

# Fit a linear regression model
model <- lm(diff ~ Age + Year + sex, data = mort_trends_ord)

# View the summary of the model to check coefficients, R-squared, etc.
summary(model)
...
mort_trends_ord$predicted_change <- predict(model, newdata = mort_trends_ord)

# Plot observed vs predicted mortality change
ggplot(mort_trends_ord, aes(x = diff, y = predicted_change)) +
  geom_point() +
  geom_abline(intercept = 0, slope = 1, color = "red") +
  labs(title = "Observed vs Predicted Mortality Change",
       x = "Observed Mortality Change",
       y = "Predicted Mortality Change") +
  theme_minimal()
...
Prevalence rates for 2018 and subsequent years.  We have not identified useful prevalence projections for the relevant diseases.

This consists of prevalence rates from GBD for 2018 to 2021 for all relevant diseases, plus rates from AIHW for specific years for diabetes, dementia \[and COPD\].  **SP note: the COPD figures from AIHW are much lower than from GBD, and this may be because the AIHW figures only cover ICD J43-J44, whereas the GBD figures cover J41-J44.  Possibly the COPD figures here are not useful and should be discarded.**
...
dem_prev_base <- read_xlsx(here("health/original/AIHW/dementia-prevalence.xlsx"),
...
dia_prev_base <- read_xlsx(here("health/original/AIHW/diabetes-prevalence.xlsx"),
...
copd_prev_base <- read_xlsx(here("health/original/AIHW/copd-prevalence-multisource.xlsx"),
...
saveRDS(prev_trends, file=here("health/processed/health_prev_trends.rds"))
