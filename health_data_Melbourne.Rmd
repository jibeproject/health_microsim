...
areas <- read.csv(here("health/original/ABS/SA2_2016_AUST.csv")) %>%
...
mort_trends_former_ord <- 
  read_excel(here("health/original/ABS/projections 2017-2066.xls"),
             sheet = "Table 5", skip = 6) %>%
  select(1, 2, 4, 14) %>%
  rename(male=`Victoria...4`, 
         female = Victoria...14) %>%
...
mort_trends_combined <- mort_trends_ord %>%
  full_join(mort_trends_former_ord,
             by = c("Year", "Age", "sex"),
             suffix = c("_2066_71", "_2017_66")) %>%
  dplyr::select(Year, Age, sex, rate_2066_71, rate_2017_66) %>%
  pivot_longer(cols = c("rate_2066_71", "rate_2017_66"),
               names_to = "type", values_to = "rate") %>%
  filter(!is.na(rate)) %>%
  mutate(type = case_when(
    type == "rate_2066_71" ~ "pred_2066-71",
...
cat("Plots have been saved in the 'plots' directory.\n")
